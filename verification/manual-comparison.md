...
and the Next candidate (`next/build` @ `feat/slice5-comparison` HEAD, post-F1

- PR-#66 review fixes; Svelte `build/` @ `93a5dd8` generation-8 content), served
  locally — never production. Facts in `manual-comparison.json`, JPEG pairs in
  `screenshots/manual-compare/{baseline,candidate}/<route>@<vp>.jpg` (60 per side:
  30 routes × 1440x900 + 390x844). Driver: `scripts/capture/manual-compare.mjs`,
  same dependency-free CDP harness as the AC7/AC9 captures.

Captured 2026-09-20 — headless Chrome 153, Node v24.21.0, loopback, no
...
| Document-shell surfaces      | `/about`, `/ko/about`, `/contact`, `/ko/contact`, `/projects`, `/ko/projects`, `/feed`, `/ko/feed` | PASS   | all render incl. KO twins; baseline emits `lang=en` on all three KO twins (full EN fallback on `/ko/contact` + `/ko/projects`; `lang=en` with localized `소셜 피드` h1 on `/ko/feed`) while the candidate emits `lang=ko` with localized h1 on all three — the approved ko-loc correction classes; shell diffs are the approved C13 prefetch/`lang` classes |
...
- **No horizontal overflow anywhere** — `scrollW` 1432/1440 identical on all 30
...
- **Zero candidate console errors on all 30 routes post-F1-fix** — the
  post-regeneration `/404` entry records `errs=0` where the pre-fix capture
  held the two `#418` hydration errors. The baseline itself emits two errors
...
| #   | Finding                                                                | Detail                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Asks                                   |
| --- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| F1  | ~~**React hydration error #418 on every candidate 404**~~ **RESOLVED** | Root cause: `404.html` renders with pathname `/_not-found` (in `ENGLISH_ONLY_PREFIXES` → no toggle), while the client hydrates with the real unmatched URL (`hasLocaleVariant` → true → toggle mounts → DOM mismatch). Fixed by threading `suppressLocaleToggle` from `global-not-found.tsx`/`global-error.tsx` through `SiteShell` → `SiteHeader` → `HeaderControls` → `LanguageToggle`; both renders now agree on the no-toggle output. Guarded by `migration:browser:notfound` (NF-01..03: 0 errors + 0 toggles on `/404`, `/does-not-exist-xyz`, `/ko/does-not-exist`) and its controls (NFC-01..03). `manual-comparison.json` was regenerated 2026-09-20 post-fix (`/404` `errs=0`) and now also covers `/ko/contact`, `/ko/projects`, `/ko/feed`. | Brandon chose fix-before-cutover; done |
| F2  | ~~**Giscus live-thread render is cross-origin**~~ **RESOLVED**         | iframe mounts on both builds with identical `src` (`term=<slug>`, `lang` per locale, `repo`, `theme=dark_dimmed`, `inputPosition=top`); C10 asserted `data-term`=slug + one-thread-per-locale at build time. **Brandon confirmed 2026-09-20** that the Giscus (GitHub Discussions) thread visibly loads on both the EN and KO `giscus-sveltekit-integration` posts on the locally served candidate (`serve-build` :4173).                                                                                                                                                                                                                                                                                                                               | Human look — done, PASS                |

## Verdict

Manual layer: **PASS, no open flags** — F1 resolved pre-cutover with a
permanent regression guard (`migration:browser:notfound`); F2 resolved by
Brandon's human look 2026-09-20 (Giscus thread visibly loads, EN + KO).
Everything else matches the baseline or sits inside an already-approved
ledger class.
