...
- markdownlint 빠른 참조 테이블 완전 제거(2026-03-09).
  `.markdownlint-cli2.jsonc`와
...
- Mermaid 섹션을 약 89줄에서 6줄로 압축(동작 규칙만 남기고 예시/테이블/체크리스트는
  제거했어요. Mermaid 선호를 강제하는 도구가 없으니까요.
...
> 아래는 독립된 일반 file 구조보다 앞서 사용한 2026년 3월 설계예요. 현재 deploy
> contract가 아니라 변화 과정을 남긴 기록이에요.
...
## 과거 shared settings chain

> 이 symlink topology는 2026년 7월에 폐기했어요. 아래 내용은 incident를
> 이해하기 위한 기록이며 다시 만들면 안 돼요.
...
> 이 섹션의 복구 명령은 과거 연결 구조에만 해당해요. 현재는 프로필별로 설정을
> 가져오고, 적용하고, 되돌리는 workflow를 사용해요.
...
SoT와 어긋나 있을 수 있어요. 그 사이에 발생한 사용자 활동(UI 권한 토글과
플러그인 활성화)은 깨진 로컬 파일에만 존재하게 돼요.
...
두 번째 케이스는 더 어려워요. 로컬 파일에 UI 토글, "항상 허용" 클릭, 플러그인
활성화 같은 사용자 의도가 쌓였고 같은 윈도우 동안 SoT도 별도로
...
나중에 추가됐기 때문에 존재해요. 대안은 두 개의 독립된 symlink
(`{personal,work} → SoT`)예요. 이 구조는 하나의 UI 깨짐을 두 프로필에 cascade하는 대신
...
> 두 번째 incident를 겪은 뒤 shared chain을 폐기했어요. 아래 복구법은 현재
> 지침이 아니라 당시 증거로 남겨요.
...
않아요. 앞 섹션의 구조적 walk는 모든 diff의 의도를 하나씩 결정하게 만드는데
...
새 개발자가 저장소를 clone하면 Claude Code 지시사항이 바로 동작하고 개인
customization은 비공개로 남아요. sync script가 AI tool 사이 drift를 막고,
중복된 공유 원칙은 전역 설정으로 올려 token 사용을 줄여요.

더 큰 교훈은 공유 지시사항과 private runtime 설정의 실패 범위가 다르다는
점이에요. 저장소 지시사항은 감사 가능한 하나의 source가 유리하고, 프로필
설정은 식별자를 확인한 독립 runtime file과 명시적인 적용 및 rollback이 더
안전해요.
