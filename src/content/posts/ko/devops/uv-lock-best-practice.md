...
새 ETL 프로젝트에 uv를 설정하다가 `.gitignore` 앞에서 멈췄어요. `uv.lock`을
커밋해야 할까, 무시해야 할까? 다른 개발자들도 쓰는 레포라서 괜히 lockfile을
커밋했다가 혼란을 줄까봐 망설여졌거든요. `.venv/`는 항상 gitignore하니까
`uv.lock`도 비슷한 거 아닌가 싶었는데, 찾아보니 완전히 다른 거였어요.
...
커밋하더라도 왜 커밋하는지 이해하지 못하면, 불필요하게 lockfile을
재생성하면서 노이즈 가득한 diff를 만들게 되고, 리뷰어들은 lockfile이 그저
쓸모없는 것이라고 생각하게 돼요.
...
lockfile 커밋은 진짜 논란의 여지가 있어요. 이것 때문에 애플리케이션에서도
논란이 있다고 착각하기 쉬운데, 애플리케이션에서는 논란의 여지가 없어요.
...
`.gitignore` 앞에서 고민했던 시간은 공식 문서의 한 문장을 읽었으면 피할 수
있었어요. 이제는 프로젝트 설정 체크리스트의 일부가 됐어요: 프로젝트 초기화,
`uv lock` 실행, `uv.lock` 커밋, 다음 단계로.
