...
프로덕션 Amplitude ETL 파이프라인에서 특정 날짜를 재처리해야 했습니다. 날짜를
하드코딩하면 이후 예약 실행을 오염시킬 위험이 있었어요. Airflow Variable은
일회성 오버라이드에는 과한 느낌이었습니다. 단 한 번의 실행에만 적용되고 이후
사라지는 파라미터 전달 방법이 필요했습니다.

## 이게 왜 중요한가요

프로덕션 Airflow에서는 특정 날짜의 데이터를 재처리하거나, 커스텀 입력으로
DAG를 테스트하거나, 실패한 실행을 디버깅하기 위해 재실행하는 일이 자주
있습니다. 문제는 정상 스케줄에 영향을 주지 않고 이걸 해야 한다는 점이에요.
하드코딩된 날짜는 코드에 남습니다. Airflow Variable은 정리하지 않으면 실행
간에 지속됩니다. 원하는 건 단 하나의 DAG 실행 동안만 존재하는 파라미터입니다.

Airflow의 `dag_run.conf`가 정확히 이 역할을 합니다. 각 수동 트리거에 JSON
설정을 함께 전달할 수 있고, 해당 실행에만 적용된 후 사라집니다. 예약 실행은
빈 config를 보고 기본값으로 폴백합니다.

## 까다로웠던 부분

첫 번째 걸림돌은 Jinja와 Python의 차이입니다. `dag_run.conf`는 Jinja 템플릿
(이중 중괄호 문법) 안에서만 동작합니다. DAG 파싱 시점에 일반 Python dict로
접근하면 조용히 실패하거나 `None`을 반환해요. 템플릿을 써야 한다는 걸
알려주는 에러 메시지도 없습니다.

날짜 범위 기본값은 복잡도를 더합니다. `yesterday_ds` 같은 단순한 기본값은
간단하지만, "10일 전 날짜를 YYYY-MM-DD 형식으로" 계산하려면
`macros.timedelta()`와 `.strftime()`을 Jinja 표현식 안에 중첩해야 합니다.
문법이 틀리면 유용한 에러가 나오지 않습니다.

config는 입력 검증도 하지 않습니다. `dag_run.conf`는 아무 JSON이나 받아요.
키 이름을 오타내면(예: `execution_date` 대신 `exec_date`) 조용히 기본값으로
폴백됩니다. 오버라이드가 안 된 것처럼 보이지만 사실 키가 매칭되지 않은
것입니다.

마지막으로 UI 트리거 버튼이 눈에 잘 띄지 않습니다. "Trigger DAG w/ config"
옵션은 기어가 달린 재생 아이콘으로, 기본 트리거 버튼이 아닙니다. 처음 쓰는
분이라면 놓치기 쉬워요.

## 패턴

Jinja 템플릿 안에서 `dag_run.conf.get()`을 폴백 기본값과 함께 사용합니다:
...
스케줄에 의해 트리거되면 `dag_run.conf`는 빈 dict이므로 `.get()`이
`yesterday_ds`를 반환합니다. 수동으로 config와 함께 트리거하면 제공된 값을
사용합니다.

## 실제 사례

단일 날짜를 처리하는 ETL DAG입니다:
...
재생 아이콘)을 클릭합니다. JSON config를 입력합니다:
...
"Trigger"를 클릭하면 해당 파라미터로 DAG가 실행됩니다.

## 격리 동작 방식

각 DAG 실행은 독립적인 `dag_run.conf`를 가집니다:
...
Run 2의 수동 config는 Run 3에 전혀 영향을 주지 않습니다. 각 실행은
독립적입니다.

## 흔한 실수

동적이어야 할 값을 하드코딩하면 안 됩니다:
...
`schedule_interval`을 config로 동적으로 만들려 하면 안 됩니다:
...
디버깅에 적합합니다. 지속되면 안 되는 일회성 파라미터 오버라이드에 맞는
도구입니다.
...
핵심 인사이트는 `dag_run.conf`가 단일 실행에 스코핑된다는 점입니다. 그 스코핑
