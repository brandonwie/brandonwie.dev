...
폰에서 테스트했더니, 탭해도 아무 반응이 없었어요. 키보드도 안 뜨고, 커서도
안 뜨고, 반응이 전혀 없었어요. Chrome DevTools 반응형 모드에서는 문제 없어
보였던 숨겨진 input 패턴이 실제 모바일 Safari에서는 완전히 깨져 있었어요.
...
데스크톱에서는 완벽하게 동작해요. 오버레이가 클릭을 무시하고, 클릭이 input으로
전달되고, input이 키 입력을 캡처해요. 하지만 모바일에서는 터치 이벤트가 다른
규칙을 따라요.
...
텍스트는 숨기지만 텍스트 커서나 선택 하이라이트는 숨기지 못해요. 일부 Safari
버전에서는 "보이지 않는" input 안에서 깜빡이는 커서가 오버레이 뒤에 유령처럼
보이기도 했어요.

수정하려면 표준 속성과 함께 vendor prefix 속성도 같이 넣어야 해요.
...
튜토리얼에는 나오지 않고, 일부러 찾지 않으면 MDN 문서에서도 바로 보이지
않아요.

### 5. 치수는 맞는데 커서 오버레이가 보이지 않았어요

이게 가장 이상했어요. 블록 커서 요소의 computed 치수는 올바른 상태였어요 --
inspector에서 10x19px을 확인했어요. 그런데 보이질 않았어요.

CSS가 잘못됐나 싶었고, 애니메이션 문제인가도 싶었고, 색상 변수가 문제인가도
의심했어요. Playwright MCP로 라이브 페이지를 직접 살펴봤어요. 같은 위치에
`z-index: 9999`인 50x50 빨간 사각형을 삽입했더니 바로 보였어요. 그리고
z-index 없는 커서 요소를 봤더니 -- 보이지 않았어요.

숨겨진 input에 `z-10`이 있었는데, 이게 stacking context를 만들어요. 오버레이
div는 DOM 순서상 input 다음에 렌더링되지만, 자체 `z-index`가 없으면 같은
방식으로 stacking context에 참여하지 못해요. 투명하지만 존재하는 input 레이어가
커서 요소 위에 그려져서, 눈에 보이는 건 없는데도 커서를 시각적으로 가리고
있었어요.

수정은 오버레이 div에 `relative z-20`을 추가하고, 클릭이 여전히 input으로
통과하도록 `pointer-events-none`도 같이 쓰는 거예요.

### 6. `inline-block`의 CSS 높이 붕괴

블록 커서는 `display: inline-block`인 `<span>`이에요. 내용은 공백 문자 하나 --
커서가 빈 문자열 끝에 있을 때 너비를 주기 위한 최소한의 내용이에요.

`inline-block` span 안의 공백 문자는 `height: 0`으로 계산돼요. 요소는
존재하고, 너비도 맞는데, 높이가 없어요. 보이질 않아요.

수정은 줄 높이에 맞는 명시적 높이를 주는 거예요.
...
Safari는 font-size가 16px 미만인 input 필드에 자동으로 줌해요. Apple은 이걸
작은 화면에서 가독성을 향상시키는 기능으로 보고 있어요. 텍스트가 투명한지는 상관
없어요. input이 숨겨져 있어도 상관없어요. 계산된 font-size가 14px이면 Safari가
줌해요.

수정은 간단해요. 텍스트가 보이지 않더라도 input에는 항상 `font-size: 16px`
이상을 쓰세요. 이 속성 하나로 커스텀 모바일 input에서 가장 흔한 불만을
제거할 수 있어요.
...
- **일반 폼 input** -- 보통의 `<input>`이나 `<textarea>`를 쓴다면 이런
  핵이 필요 없어요. 브라우저가 터치 이벤트를 알아서 처리해요.
- **데스크톱 전용 애플리케이션** -- z-index와 font-size 우회법은 모바일 브라우저
  특유의 문제를 위한 거라 데스크톱에서는 불필요한 복잡성만 더해요.
- **접근성이 최우선인 폼** -- 실제 input을 숨기고 시각적 오버레이를 보여주면
  스크린 리더가 혼란스러워할 수 있어요. 접근성이 최우선이면 네이티브 input에 CSS
  스타일링을 적용하세요.
- **ContentEditable 접근법** -- `contenteditable`을 쓴다면 터치 이벤트와
  z-index 문제가 해당되지 않아요.
...
하나라도 빠지면 모바일에서 뭔가 깨져요. 데스크톱 테스트는 전부 통과해요.
실제 폰을 들어봐야 알 수 있어요.
