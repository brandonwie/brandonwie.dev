...
description: 모든 라우트 핸들러에서 `Depends(get_current_user)`를 반복하면 보일러플레이트가 생기고 불일치가 발생하기 쉬워요
...
개발자가 여럿 붙고 코드베이스가 커지면 "다른 라우터에서 이 패턴을 복사해"가
타입 미스매치와 미묘한 버그가 쌓이는 경로가 돼요.
...
메타데이터(FastAPI 인젝터용)가 같이 들어 있어요. `current_user: CurrentUser`를
...
`AttributeError: 'NoneType' has no attribute 'sub'`가 나요. 실제 실수가 있는
라우터와는 한참 떨어진 지점이죠.
...
어느 쪽을 골라야 할지 헷갈려요. `Annotated`는 Python 3.9부터 `typing`에
들어왔고(그 이전 버전은 `typing_extensions`가 필요해요), FastAPI가 메타데이터를
읽기 시작한 건 0.95.0부터예요.
...
- **FastAPI가 아닌 프레임워크**: `Annotated[..., Depends()]` 패턴은
  FastAPI 전용이에요. Flask, Django 등 다른 프레임워크는 DI 메커니즘이 아예
  달라요.

기억할 규칙은 하나예요. `Annotated`를 항상 맨 바깥에 두세요. 공유 dependency는
`deps.py`에 `Annotated` type alias로 정의하고, 사용자 ID는 라우터 경계에서
변환하면 돼요. 선택적으로 만들어야 한다면 `Optional`을 alias 안쪽으로 옮기고요.
