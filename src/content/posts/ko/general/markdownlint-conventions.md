...
description: >-
  ...
  scope에서만 표면화되는 두 가지 pre-commit 함정, 그리고 18-rule custom config를 한 줄 extends + 5개
...
테이블 포맷은 두 개의 룰로 나뉘어 있어요. 둘 다 pipe 문자에 대한 얘기라서
...
v0.41.1 기준으로 `style` 값은 `consistent`(기본값 — 문서의 첫 번째 테이블이
나머지의 스타일을 정해요)와 명시적인 네 가지예요.
...
추가됐고, 값의 종류가 아예 달라요 — `any`(기본값), `aligned`, `compact`,
`tight`.
...
저는 aligned 컬럼 + leading/trailing pipe로 정착했어요.
...
컬럼 스타일 룰 하나가 7,500개 에러 중 3,600개 이상을 차지했어요. 수정은
기계적이었지만 컨벤션이 먼저 있어야 했어요.
...
위 같은 18-rule custom config는 시간이 지나면서 no-op이거나 redundant이거나 조용히 broken인 entry들이 쌓여요. 몇 달 같이 살아본 후 저는 upstream `style/all` preset + 적은 수의 documented carve-out으로 마이그레이션했어요. 마이그레이션은 sprawling per-rule config를 한 줄 `extends:` + 몇 개의 explicit exception으로 collapse했고, ineffective override 뒤에 숨어 있던 36개 MD040 에러를 표면화시켰어요.
...
위 두 줄은 18개 커스터마이징이 "default 대비 no-op"이었다면 결과가 같아야 했어요. 그런데 기존 config가 MD013 false(line-length), MD024 siblings_only=true, MD025 front_matter_title=""을 통해 약 11,000개 실패를 막고 있었던 거예요 — 즉, 그 override들은 군더더기가 아니라 무게를 받치고 있었어요.
...
override가 linter를 조종하는 것처럼 보이지만 실제로는 의도한 일을 아무것도 안 할 수 있어요. 경고도 없어요 — 파일은 파싱되고, 실행은 성공하고, config는 config처럼 읽혀요. 제 마이그레이션 이전 config에 이런 게 두 개 있었어요.
...
MD060 entry가 가장 명확한 케이스예요. "테이블마다 consistent한 스타일이면 accept"라는 주석을 달아뒀는데, `any`가 하는 일에 대한 설명으로는 맞아요. 다만 제가 **바꾸고 있는** 것에 대한 설명은 아니었어요. 아무것도 안 썼어도 `any`였을 테니까요.
...
sprawling 18-rule custom config를 한 줄 `extends:` + 5개 documented carve-out으로 대체했고,
