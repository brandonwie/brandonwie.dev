...
**A deferred tool wrongly declared missing (historical, pre-2.1.178).** This one was particularly insidious. A session concluded that `TeamCreate` did not exist, because it was a deferred tool that stayed out of the visible tool list until `ToolSearch` loaded it. My global CLAUDE.md was updated to say "use Agent tool instead," which pushed every later session into background subprocesses instead of split-pane teams. The correction was straightforward: use `ToolSearch` to check for deferred tools before concluding they are absent. The damage was sessions of degraded behavior where I thought teams were running and was getting background subagents instead.

**Deferred tool visibility bias (historical, pre-2.1.178).** Even after documenting `TeamCreate` correctly, Claude kept reaching for the `Agent` tool without `team_name`, which on those releases ran in-process with no split panes. The root cause was visibility: `Agent` sat in the primary tool list while `TeamCreate` needed a `ToolSearch` call first, and a model defaults to the tool it can already see. My workaround was a `SessionStart` hook (`session-start-team-preload.py`) that reminded the session to preload `TeamCreate`, with an explicit `ToolSearch` → `TeamCreate` → `Agent(team_name)` sequence in the advisory text.

That workflow is now history, not advice. The official docs state that both `TeamCreate` and `TeamDelete` were removed in v2.1.178, and that the `team_name` input on the `Agent` tool is accepted but ignored. Do not copy the three-step sequence into a current setup. What generalizes is the bias underneath it: a model reaches for whatever tool is visible, so when the right tool needs an extra load step, something in the session has to say so.
