...
I was building a calendar sync feature that received thousands of event IDs from Google Calendar and needed to look them up in the application's own database. The query used TypeORM's `In([...])` operator, which worked fine in development with dozens of IDs. A sync for a heavy calendar user could return thousands of IDs though, and the ceiling I found while sizing that query was hard: beyond a few thousand IDs planning slows noticeably, and at 65,535 the query fails outright with a cryptic protocol-level error.
...
- **500** was the sweet spot here: B-tree index scans with sub-millisecond planning
...
I chose batching because it works with TypeORM's `find()` API without dropping to raw SQL. The existing finder was already composed with other query-builder conditions, and switching to `ANY(array)` would have meant rewriting the query.
...
For a one-shot load, `COPY ... FROM STDIN` sidesteps the question entirely. It streams and uses no bind parameters at all, so the columns × rows arithmetic never applies. The PostgreSQL docs on [populating a database](https://www.postgresql.org/docs/current/populate.html) put the performance side plainly: loading a large number of rows with `COPY` is almost always faster than `INSERT`, even with `PREPARE` and batching. I never benchmarked the two side by side myself, so I'd treat that as the documented direction rather than a number I can vouch for. `COPY` also bypasses the ORM path, so it fits an import job better than application code.
