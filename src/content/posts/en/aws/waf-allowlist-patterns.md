...
description: >-
  Block-by-default WAF approach using route allowlisting. Unknown routes are
  blocked automatically, which is stronger security than a blocklist.
...
**The recommendation:** use allowlist for APIs with known, stable routes. If your API serves `/users`, `/orders`, `/invoices`, and `/socket.io`, those are the only paths that should reach your application. Everything else is noise.
...
For most public API routes that is an acceptable trade: you control the handler, and the allowlist already rejected everything else. It matters most for internal or privileged endpoints -- an `/admin/*` namespace, say -- where allowlisting means managed rules and rate limiting never apply, so the application itself has to carry that protection. Factor that into the threat model before adding the allow entry.
...
There is a second reason to reach for `EXACTLY`, and it is easy to overlook: `STARTS_WITH` matches more than the path and its subtree. `STARTS_WITH "/foo"` matches `/foo` and `/foo/bar`, but it also matches sibling prefixes like `/foo-bar` and `/foobar`. For a single-route endpoint with no subtree -- say, a lone `POST /reindex` -- allowlisting it with `STARTS_WITH` quietly admits every future sibling path that happens to share the prefix, `/reindex-all` included. `EXACTLY "/reindex"` closes that leak.

The trade-off is that `EXACTLY` also rejects the trailing-slash variant (`/reindex/`) and any subpath added later -- each one then needs its own entry, the same discipline as maintaining `/health` alongside a hypothetical `/health/websocket`. The decision rule I settled on:
...
Use this for API versioning patterns like `/v1/api/users` or `/v2/api/orders`. Be careful with `CONTAINS` -- it is the least restrictive constraint and can match more broadly than you intend.
...
One thing to know before you grep that output: the literal path may not be in it. Each `ByteMatchStatement.SearchString` came back base64-encoded for me, so searching the JSON for `/reindex` found nothing even though the rule was live.

That is consistent with the API contract -- `SearchString` is typed as a base64-encoded binary data object, and the docs say the CLI and SDKs handle the encoding for you on the way *in*. What the docs do not state anywhere I could find is how the value is rendered on the way *out*. So treat this as observed behavior, not a guarantee: it is what AWS CLI v2 printed for me in June 2026, and worth confirming in your own environment before you script around it.

Either grep for the encoded value (`/reindex` encodes to `L3JlaW5kZXg=`) or base64-decode the field:

```bash
aws wafv2 get-web-acl --name app-prod-waf --scope REGIONAL --id <id> \
  --region ap-northeast-2 --query 'WebACL.Rules' --output json \
  | grep -c "L3JlaW5kZXg="   # 1 = present
```
...
The non-obvious one is the 504: it is never a WAF block. By the time you see it, the request already cleared the allowlist and reached the backend, so the problem is the handler or its egress -- not the WAF rules. A 403 with your custom block body is the opposite signal: the path is missing from the allowlist, and the fix is to add the corresponding `byte_match` entry (for versioned routes, watch for the `/v2/` prefix gotcha covered below).
...
One issue that caught me off guard: `STARTS_WITH "/reports"` does NOT match `/v2/reports`. The URI path literally starts with `/v2/`, not `/reports`. This is obvious in hindsight, but when you are adding a new versioned API route, it is easy to assume that the existing `/reports` allowlist entry covers all versions.
...
Without the explicit `/v2/reports` entry, requests silently return 403 in production. The tricky part is that dev environments often blanket-allow `/v2/*` via regex, so the route works perfectly in dev and only fails in prod where explicit rules are used.

**Checklist for new v2 routes:** when you add a v2 controller in the backend, add the matching entry to whichever Terraform file holds the production web ACL in the same change. If your dev WAF blanket-allows `/v2/*`, the route works there automatically -- which is exactly why you will not catch this in development.

### The Inverse Case: Sub-Paths Are Already Covered

The versioned-route gotcha has an inverse worth checking before you touch Terraform at all. A `STARTS_WITH` entry covers the entire namespace beneath it: if `/invoices` is already allowlisted, a new `/invoices/archive` route is already allowed, and adding a dedicated entry for it is redundant -- extra WCU (web ACL capacity units) spent for zero change in behavior.
...
6. **Versioned routes need separate entries.** `STARTS_WITH "/reports"` does not match `/v2/reports`. Each version prefix requires its own allowlist statement.
...
9. **Sub-paths of an allowed namespace are already covered.** A `STARTS_WITH` entry on `/invoices` already allows `/invoices/archive`, so a dedicated entry adds cost without changing behavior. It is the inverse of the versioned-route rule: version and root prefixes need their own entries, sub-paths do not. Verify with the `.tf` and a `terraform plan` no-op before adding a rule.
