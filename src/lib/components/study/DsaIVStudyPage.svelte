<script lang="ts">
	import PatternMatchVisualizer from '$lib/components/study/PatternMatchVisualizer.svelte';
	import GraphTraversalVisualizer from '$lib/components/study/GraphTraversalVisualizer.svelte';
	import MstVisualizer from '$lib/components/study/MstVisualizer.svelte';
	import LcsTableVisualizer from '$lib/components/study/LcsTableVisualizer.svelte';
	...
	import { getDsaIVContent, type StudyLocale } from '$lib/data/study';

	let { locale = 'en' }: { locale?: StudyLocale } = $props();

	const content = $derived(getDsaIVContent(locale));
...
<StudySeoHead {pageTitle} description={content.metaDescription} basePath="/study/dsa-iv" {locale} />
