<script lang="ts">
	...
	import { hasLocaleVariant, localeOf, pathForLocale } from '$lib/data/nav';
	...
	// English-only routes (e.g. /talks) have no counterpart to toggle to. Rendering
	// the link anyway would both mislead the reader and point the SSG crawler at a
	// path that does not exist, failing the build.
	const translatable = $derived(hasLocaleVariant(page.url.pathname));
