...
import type { PatternMatchCopy } from '../../data/study';
...
export default function PatternMatchVisualizer({ copy }: { copy: PatternMatchCopy }) {
	...
	const roleBadge = frame.role === null ? '' : copy.roleLabels[frame.role];
			...
			<div className="mt-5 overflow-x-auto">
				...
				>
					<text
						...
						className="fill-faint font-mono text-[8px] uppercase tracking-wider"
					>
						{copy.textLabel}
					</text>
					<text
						...
						return (
							<g key={index}>
								...
								<text
									...
									textAnchor="middle"
									dominantBaseline="central"
									className={`font-mono text-[13px] ${charClass(role)}`}
								>
									{char}
								</text>
							</g>
					...
					{roleBadge && (
						<text
							...
							y="47"
							textAnchor="start"
							className={`font-mono text-[8px] uppercase tracking-wider ${charClass(frame.role ?? 'idle')}`}
						>
							{roleBadge}
						</text>
					)}
					...
					{patternChars.map((char, index) => {
						...
						const px = cellX(frame.align + index);
						return (
							<g key={index}>
								<rect
									x={px}
			...
			<div className="mt-5">
				...
				<div className="mt-2 flex flex-wrap gap-2">
					{tableEntries.map((entry) => (
						<span
							key={entry.key}
							className={`border px-2.5 py-1 font-mono text-sm ${entryClass(entry.key)}`}
						>
			...
			<div className="mt-5">
				...
				<div className="mt-2 flex flex-wrap gap-2">
					{LEGEND.map((role) => (
						<span
							key={role}
							className={`border px-2.5 py-1 font-mono text-[11px] uppercase ${legendClass(role)}`}
						>
